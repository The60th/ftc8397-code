...
public class CryptoNavNew {

    //Control of debug logging.

    protected static final String TAG = "CryptoNav";
    private static final boolean DEBUG_INIT = true;
    private static final boolean DEBUG_GETBLOBS = true;
    protected static final boolean DEBUG_INITRAILS = true;
    private static final boolean DEBUG_FILTER = true;
    private static final boolean DEBUG_UPDATE = true;
    protected static final boolean DEBUG_UPDATELOC = true;

    //Dimensions of the CryptoBox in cm

    protected static final float RAIL_THICKNESS_CM = 0.75f * 2.54f;
    protected static final float RAIL_DEPTH_CM = 4.0f * 2.54f;
    protected static final float RAIL_SPACING_CM = 7.63f * 2.54f;
    ...
    protected static Blob[] rails = new Blob[4];
    ...
    protected static float focalLength;

    //principal X value of reduced image

    protected static float principalX;
    ...
    //Initializes imaging parameters, including the properties (width, height, principal point, focal length) of
    //raw images coming from the camera; the crop (range) that will be selected from the raw image,
    //and the factor by which the cropped image will be reduced (sample ratio)
    ...
    //Overload of initParams in which the raw image width and height, principal point, and focal length are
    //obtained automatically from the camera.
    ...
    //Given a raw image (rawImageBytes), obtain a list of blobs of color "teamColor"
    ...
    //lastInterRailDistance also initialized, if there are at least two visible rails. The startSide parameter
    //is a guarantee: if LEFT, then the left-most cryptobox rail must be visible in the image; if RIGHT, then the
    //right-most cryptobox rail must be visible. Note: if all 4 cryptobox rails are visible, then it doesn't matter
    //whether RIGHT or LEFT is used.
    ...
    public static boolean initializeRails(byte[] rawImageBytes, Side startSide) {
        ...
        //Obtain list of blobs from the raw image, then filter and sort to get blobs
        //representing the cryptobox rails.
        ...
        //If the number of rails found is 0 or >4, then we've failed.
        ...
        //Take the blobs from railList, and but them in an array of blobs (rails). This array has exactly
        //four elements, corresponding to the four rails, from left to right. If there are rails on one
        //side that are not currently visible, the corresponding elements in rails will be null.
    ...
    Note that the parameter, a list of blobs, is filtered and sorted (by increasing X value) in place.
    */
   ...
   /*
    ...
    of two furthest-apart rails, and  camera heading phiPrime (obtained from gyro and passed in as argument) to determine the location of the
    ...
    Note: We are not seeing the rails from end-on. Each rail is seen from a different angle, dependent on robot position
    and orientation. We don't know which point within a blob represents the front-center of the rail. But, if the robot
    is located to the right of the rail, its a reasonable assumption that the right edge of the blob represents the right-back
    edge of the rail. Similar reasoning is used if the robot is located to the left of the rail. So we're using
    the right/left back edges of the rails a the known points for navigation.
